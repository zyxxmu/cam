...
class LlamaAttention_sink(nn.Module):
    ...
    def forward(
        ...
        if self.previous_scores == None:
            self.start_budget = int(attn_weights.shape[-1]*self.start_budget_ratio)
            self.recent_budget = int(attn_weights.shape[-1]*self.recent_budget_ratio)
        ...
        attn_mask = torch.ones(attn_weights.shape[1], attn_weights.shape[-1]+1).to(dtype_attn_weights).to(attn_weights_devices)
