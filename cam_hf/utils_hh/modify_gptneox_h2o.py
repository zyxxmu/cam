...
__all__ = ['convert_kvcache_gpt_neox_h2o', 'GPTNeoXAttention_Mask_h2o']


class GPTNeoXAttention_Mask_h2o(nn.Module):
...
def convert_kvcache_gpt_neox_h2o(model, config):

    for name, module in reversed(model._modules.items()):

        if len(list(module.children())) > 0:
            model._modules[name] = convert_kvcache_gpt_neox_h2o(module, config)

        if isinstance(module, GPTNeoXAttention):
            model._modules[name] = GPTNeoXAttention_Mask_h2o(config)
