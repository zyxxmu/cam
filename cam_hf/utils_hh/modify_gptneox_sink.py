...
__all__ = ['convert_kvcache_gpt_neox_sink', 'GPTNeoXAttention_Mask_sink']


class GPTNeoXAttention_Mask_sink(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        attn_scores = torch.baddbmm(
            ...
            key.transpose(1, 2),
            beta=1.0,
            alpha=(torch.tensor(1.0, dtype=self.norm_factor.dtype, device=self.norm_factor.device) / self.norm_factor).to(torch.float32),
        )
        ...
        attn_mask = torch.ones(num_attention_heads, attn_weights.shape[-1]+1).to(dtype_attn_weights).to(attn_weights_devices)

        attn_mask[:, self.start_budget:attn_weights.shape[-1]+1-self.recent_budget] = 0
...
def convert_kvcache_gpt_neox_sink(model, config):

    for name, module in reversed(model._modules.items()):

        if len(list(module.children())) > 0:
            model._modules[name] = convert_kvcache_gpt_neox_sink(module, config)

        if isinstance(module, GPTNeoXAttention):
            model._modules[name] = GPTNeoXAttention_Mask_sink(config)
