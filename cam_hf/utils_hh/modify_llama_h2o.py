...
__all__ = ['convert_kvcache_llama_h2o', 'LlamaAttention_h2o']


class LlamaAttention_h2o(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.heavy_budget_ratio = config.start_ratio
    ...
    def forward(
        ...
        if not self.previous_scores == None:
            ...
            self.heavy_budget = int(current_scores_sum.shape[-1]*self.heavy_budget_ratio)
            self.recent_budget = int(current_scores_sum.shape[-1]*self.recent_budget_ratio)
...
def convert_kvcache_llama_h2o(model, config):

    for name, module in reversed(model._modules.items()):

        if len(list(module.children())) > 0:
            model._modules[name] = convert_kvcache_llama_h2o(module, config)

        if isinstance(module, LlamaAttention):
            model._modules[name] = LlamaAttention_h2o(config)
