...
__all__ = ['convert_kvcache_opt_heavy_recent', 'OPTAttention_Mask_sink']


class OPTAttention_Mask_sink(nn.Module):
    ...
    def forward(
        ...
        attn_mask[:, self.heavy_budget:attn_weights.shape[-1]+1-self.recent_budget] = 0
...
def convert_kvcache_opt_sink(model, config):

    for name, module in reversed(model._modules.items()):

        if len(list(module.children())) > 0:
            model._modules[name] = convert_kvcache_opt_sink(module, config)

        if isinstance(module, OPTAttention):
            model._modules[name] = OPTAttention_Mask_sink(
