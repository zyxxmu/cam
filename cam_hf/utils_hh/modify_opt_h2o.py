...
__all__ = ['convert_kvcache_opt_h2o', 'OPTAttention_Mask_h2o']


class OPTAttention_Mask_h2o(nn.Module):
...
def convert_kvcache_opt_h2o(model, config):

    for name, module in reversed(model._modules.items()):

        if len(list(module.children())) > 0:
            model._modules[name] = convert_kvcache_opt_h2o(module, config)

        if isinstance(module, OPTAttention):
            model._modules[name] = OPTAttention_Mask_h2o(
                embed_dim=module.embed_dim,
                num_heads=config.num_attention_heads,
                heavy_ratio = config.start_ratio,
